...
public class BDTable {
    ...
    public Connection connecter() throws Exception {
        try {
            ...
            return con = DriverManager.getConnection("jdbc:postgresql://localhost:5432/restaurant", "postgres", "prom13123");
        ...
        try {
            ...
            while (res2.next()) {
                ...
                for (int i = 0; i < method.length; i++) {
                    ...
                    if (classe[0].getSimpleName().equals("Integer") || classe[0].getSimpleName().equals("int") || classe[0].getSimpleName().equals("serial")) {
                        retour.getClass().getMethod("set" + toUpperFirst(fields[i].getName()), classe).invoke(retour, res2.getInt(id));
                    }
                    if (classe[0].getSimpleName().equals("Double") || classe[0].getSimpleName().equals("double")) {
                        retour.getClass().getMethod("set" + toUpperFirst(fields[i].getName()), classe).invoke(retour, res2.getDouble(id));
                    }
                    if (classe[0].getSimpleName().equals("String")) {
                        retour.getClass().getMethod("set" + toUpperFirst(fields[i].getName()), classe).invoke(retour, res2.getString(id));
                    }
                    if (classe[0].getSimpleName().equals("Date")) {
                        retour.getClass().getMethod("set" + toUpperFirst(fields[i].getName()), classe).invoke(retour, res2.getDate(id));
                    }
                    id++;
                }
                id = 1;
